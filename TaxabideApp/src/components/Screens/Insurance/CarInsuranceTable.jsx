...
// Add a helper function for URL handling like in ClientsTable
...
const CarInsuranceTable = ({ navigation }) => {
  ...
  const filteredData = insuranceData.filter(item => {
    ...
    return (
      ...
      (item.c_i_n_registration_no && item.c_i_n_registration_no.toLowerCase().includes(searchTerms))
    );
  });
        ...
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            ...
            <ScrollView style={styles.modalBody}>
              ...
              {selectedInsurance.c_i_n_location && (
                <View style={styles.detailSection}>
                  <Text style={styles.sectionTitle}>Location Information</Text>
                  <View style={styles.locationDetailContainer}>
                    <MaterialIcons name="location-on" size={24} color="#e53935" />
                    <Text style={styles.locationDetailText}>{selectedInsurance.c_i_n_location}</Text>
                  </View>
                </View>
              )}
              ...
              {selectedInsurance.c_i_n_rc && (
                <View style={styles.detailSection}>
                  ...
                  <View style={styles.imageContainer}>
                    <Image 
                      source={{ uri: getFullUrl(`uploads/${selectedInsurance.c_i_n_rc}`) }} 
                    ...
                    <TouchableOpacity 
                      ...
                      onPress={() => viewFullImage(getFullUrl(`uploads/${selectedInsurance.c_i_n_rc}`))}
                    >
              ...
              {selectedInsurance.c_i_n_pan_photo && (
                <View style={styles.detailSection}>
                  ...
                  <View style={styles.imageContainer}>
                    <Image 
                      source={{ uri: getFullUrl(`uploads/${selectedInsurance.c_i_n_pan_photo}`) }} 
                      style={styles.documentImage}
                      resizeMode="contain"
                      onError={(e) => console.log('Modal image error:', e.nativeEvent.error)}
                    />
                    <TouchableOpacity 
                      style={styles.fullscreenButton}
                      onPress={() => viewFullImage(getFullUrl(`uploads/${selectedInsurance.c_i_n_pan_photo}`))}
                    >
                      <MaterialIcons name="fullscreen" size={24} color="#fff" />
                    </TouchableOpacity>
                  </View>
              ...
              {selectedInsurance.c_i_n_vehicle_photo && (
                <View style={styles.detailSection}>
                  ...
                  <View style={styles.imageContainer}>
                    <Image 
                      source={{ uri: getFullUrl(`uploads/${selectedInsurance.c_i_n_vehicle_photo}`) }} 
                      style={styles.documentImage}
                      resizeMode="contain"
                      onError={(e) => console.log('Modal image error:', e.nativeEvent.error)}
                    />
                    <TouchableOpacity 
                      style={styles.fullscreenButton}
                      onPress={() => viewFullImage(getFullUrl(`uploads/${selectedInsurance.c_i_n_vehicle_photo}`))}
                    >
                      <MaterialIcons name="fullscreen" size={24} color="#fff" />
                    </TouchableOpacity>
                  </View>
              ...
              {selectedInsurance.c_i_n_aadhar && (
                <View style={styles.detailSection}>
                  <Text style={styles.sectionTitle}>Aadhar Photo</Text>
                  <View style={styles.imageContainer}>
                    <Image 
                      source={{ uri: getFullUrl(`uploads/${selectedInsurance.c_i_n_aadhar}`) }} 
                      style={styles.documentImage}
                      resizeMode="contain"
                      onError={(e) => console.log('Modal image error:', e.nativeEvent.error)}
                    />
                    <TouchableOpacity 
                      style={styles.fullscreenButton}
                      onPress={() => viewFullImage(getFullUrl(`uploads/${selectedInsurance.c_i_n_aadhar}`))}
                    >
                      <MaterialIcons name="fullscreen" size={24} color="#fff" />
                    </TouchableOpacity>
                  </View>
              ...
              <View style={styles.detailSection}>
                ...
                <DetailItem 
                  label="Client ID" 
                  value={selectedInsurance.c_i_n_client_id || 
                         selectedInsurance.client_id || 
                         selectedInsurance.c_client_id || 
                         '#' + (selectedInsurance.id || '')} 
                />
                <DetailItem 
                  label="Add Date" 
                  value={selectedInsurance.created_at || 
                         selectedInsurance.c_i_n_add_date || 
                         selectedInsurance.add_date || 
                         selectedInsurance.c_add_date || 'N/A'} 
                />
              </View>
            </ScrollView>
          </View>
  ...
  useEffect(() => {
    if (insuranceData.length > 0) {
      ...
      console.log('Client ID value:', 
        insuranceData[0].c_i_n_client_id || 
        insuranceData[0].client_id || 
        insuranceData[0].c_client_id || 
        '#' + (insuranceData[0].id || '')
      );
      console.log('Add date value:', 
        insuranceData[0].created_at || 
        insuranceData[0].c_i_n_add_date || 
        insuranceData[0].add_date || 
        insuranceData[0].c_add_date || 'N/A'
      );
    }
  }, [insuranceData]);
  ...
  return (
    <SafeAreaView style={styles.container}>
        ...
        >
          <View style={styles.tableContainer}>
            <View style={styles.tableHeader}>
              ...
              <Text style={[styles.headerCell, styles.headerCellM]}>vehicle_type</Text>
              <Text style={[styles.headerCell, styles.headerCellM]}>policy_type</Text>
              <Text style={[styles.headerCell, styles.headerCellM]}>registration_date</Text>
              <Text style={[styles.headerCell, styles.headerCellM]}>registration_no</Text>
              <Text style={[styles.headerCell, styles.headerCellS]}>rc</Text>
              <Text style={[styles.headerCell, styles.headerCellS]}>claim</Text>
              <Text style={[styles.headerCell, styles.headerCellM]}>old_insurance_expiry_date</Text>
              <Text style={[styles.headerCell, styles.headerCellM]}>Nominee Aame</Text>
            ...
            >
              {filteredData.length > 0 ? (
                currentEntries.map((item, index) => (
                  <TouchableOpacity 
                    key={item.id || item.c_i_n_id || index} 
                  ...
                  >
                    ...
                    <Text style={[styles.cell, styles.cellM]}>
                      {getFieldValue(item, ['c_i_n_name', 'name', 'c_name'])}
                    </Text>
                    <Text style={[styles.cell, styles.cellL]}>
                      {getFieldValue(item, ['c_i_n_email', 'email', 'c_email'])}
                    </Text>
                    <Text style={[styles.cell, styles.cellM]}>
                      {getFieldValue(item, ['c_i_n_phone', 'phone', 'c_phone', 'mobile'])}
                    </Text>
                    <Text style={[styles.cell, styles.cellM]}>
                      {getFieldValue(item, ['c_i_n_vehicle_type', 'vehicle_type', 'c_vehicle_type'])}
                    </Text>
                    <Text style={[styles.cell, styles.cellM]}>
                      {getFieldValue(item, ['c_i_n_policy_type', 'policy_type', 'c_policy_type'])}
                    </Text>
                    <Text style={[styles.cell, styles.cellM]}>
                      {getFieldValue(item, ['c_i_n_registration_date', 'registration_date', 'c_registration_date'])}
                    </Text>
                    <Text style={[styles.cell, styles.cellM]}>
                      {getFieldValue(item, ['c_i_n_registration_no', 'registration_no', 'c_registration_no'])}
                    </Text>
                    <View style={[styles.cell, styles.cellS]}>
                      <SimpleImage imageUrl={
                        getFieldValue(item, ['c_i_n_rc', 'rc', 'c_rc']) ? 
                        getFullUrl(`uploads/${getFieldValue(item, ['c_i_n_rc', 'rc', 'c_rc'])}`) : 
                        null
                      } />
                    </View>
                    <Text style={[styles.cell, styles.cellS]}>
                      {getFieldValue(item, ['c_i_n_claim', 'claim', 'c_claim'])}
                    </Text>
                    <Text style={[styles.cell, styles.cellM]}>
                      {getFieldValue(item, ['c_i_n_old_insurance_expiry_date', 'old_insurance_expiry_date', 'c_old_insurance_expiry_date'])}
                    </Text>
                    <Text style={[styles.cell, styles.cellM]}>
                      {getFieldValue(item, ['c_i_n_nominee_name', 'nominee_name', 'c_nominee_name'])}
                    </Text>
                    <Text style={[styles.cell, styles.cellS]}>
                      {getFieldValue(item, ['c_i_n_nominee_age', 'nominee_age', 'c_nominee_age'])}
                    </Text>
                    <Text style={[styles.cell, styles.cellM]}>
                      {getFieldValue(item, ['c_i_n_nominee_relation', 'nominee_relation', 'c_nominee_relation'])}
                    </Text>
                    <View style={[styles.cell, styles.cellS]}>
                      <SimpleImage imageUrl={
                        getFieldValue(item, ['c_i_n_vehicle_photo', 'vehicle_photo', 'c_vehicle_photo']) ? 
                        getFullUrl(`uploads/${getFieldValue(item, ['c_i_n_vehicle_photo', 'vehicle_photo', 'c_vehicle_photo'])}`) : 
                        null
                      } />
                    </View>
                    <View style={[styles.cell, styles.cellS]}>
                      <SimpleImage imageUrl={
                        getFieldValue(item, ['c_i_n_aadhar', 'aadhar', 'c_aadhar', 'c_i_n_aadhar_photo', 'aadhar_photo', 'c_aadhar_photo']) ? 
                        getFullUrl(`uploads/${getFieldValue(item, ['c_i_n_aadhar', 'aadhar', 'c_aadhar', 'c_i_n_aadhar_photo', 'aadhar_photo', 'c_aadhar_photo'])}`) : 
                        null
                      } />
                    </View>
                    <View style={[styles.cell, styles.cellS]}>
                      <SimpleImage imageUrl={
                        getFieldValue(item, ['c_i_n_pan_photo', 'pan_photo', 'c_pan_photo']) ? 
                        getFullUrl(`uploads/${getFieldValue(item, ['c_i_n_pan_photo', 'pan_photo', 'c_pan_photo'])}`) : 
                        null
                      } />
                    </View>
                    <Text style={[styles.cell, styles.cellM, styles.clientIdCell]}>
                      {getFieldValue(item, ['c_i_n_client_id', 'client_id', 'c_client_id'], '#' + (item.id || ''))}
                    </Text>
                    <Text style={[styles.cell, styles.cellM, styles.clientIdCell]}>
                      {getFieldValue(item, ['created_at', 'c_i_n_add_date', 'add_date', 'c_add_date'])}
                    </Text>
                  </TouchableOpacity>
                ))
              ) : (
      ...
      {/* Add this below the filters section and above the table section */}
