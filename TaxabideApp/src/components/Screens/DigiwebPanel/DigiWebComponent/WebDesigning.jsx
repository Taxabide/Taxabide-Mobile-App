...
const WebDesigning = () => {
  ...
  const fetchClients = async (currentUserId) => {
    try {
      ...
      try {
        ...
        try {
          const alternateResponse = await fetch(`https://taxabide.in/api/getClients.php?user_id=${userIdToUse}`);
          
          if (alternateResponse.ok) {
            const alternateText = await alternateResponse.text();
            const alternateData = JSON.parse(alternateText);
        ...
        setClientError('Invalid response format from server');
        return;
      }
      
      if (!response.ok) {
        setClientError(`Server error: ${response.status}`);
        return;
      }
      
      if (data.status === 'success' && Array.isArray(data.data)) {
        if (data.data.length === 0) {
          setClientError('No clients available');
        } else {
          setClientsList(data.data);
        }
      } else {
        setClientError('Invalid data format received from server');
      }
    } catch (error) {
  ...
  const handleSubmit = async () => {
    ...
    if (!userId) {
      Alert.alert(
        ...
        [
          ...
          {
            text: 'Login',
            onPress: () => navigation.navigate('SignIn', { returnTo: 'WebDesigning' })
          }
        ]
      );
      return;
    ...
    try {
      ...
      try {
        const apiUrl = `https://taxabide.in/api/tdws-web-designing-api.php?uid=${userId}&user_id=${userId}&t_d_user_id=${userId}`;
              ...
              <View style={styles.loginPromptContainer}>
                ...
                <TouchableOpacity
                  ...
                  onPress={() => {
                    ...
                    navigation.navigate('SignIn', {returnTo: 'WebDesigning'});
                  }}>
                ...
                {filteredClients.map((client) => (
                  ...
                  >
                    <Text style={styles.dropdownItemText}>
                      {client.c_name} {client.c_email ? `, Email: ${client.c_email}` : ''}
                    </Text>
                  </TouchableOpacity>
                ))}
              </ScrollView>
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        <View style={styles.container}>
          <Text style={styles.heading}>
            Tulyarth DigiWeb Services - Web Designing
          </Text>
          ...
          <View style={styles.pickerContainer}>
            ...
            >
              ...
              <Picker.Item label="Web Design Basic" value="Web Design Basic" />
              <Picker.Item label="Web Design Premium" value="Web Design Premium" />
              <Picker.Item label="UI/UX Design" value="UI/UX Design" />
              <Picker.Item label="Logo Design" value="Logo Design" />
              <Picker.Item label="Branding Package" value="Branding Package" />
              <Picker.Item label="Banner Design" value="Banner Design" />
            </Picker>
          </View>
