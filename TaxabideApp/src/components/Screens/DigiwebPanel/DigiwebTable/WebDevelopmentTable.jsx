...
const WebDevelopmentTable = () => {
  ...
  useEffect(() => {
    ...
    getUserId().then(id => {
      if (id) {
        ...
        // Then fetch web development data with a slight delay to ensure client data is loaded
        setTimeout(() => {
  ...
  const fetchWebDevelopmentData = async currentUserId => {
    try {
      ...
      let apiUrls = [
        `https://taxabide.in/api/tdws-web-development-list-api.php?user_id=${userIdToUse}&page=${currentPage}&limit=${entriesPerPage}`,
        `https://taxabide.in/api/tdws-web-development-api.php?user_id=${userIdToUse}&page=${currentPage}&limit=${entriesPerPage}`,
      ];
      ...
      try {
        ...
        try {
            ...
            const processedData = await Promise.all(
              data.data.map(async item => {
                ...
                if (!clientDetails.name && !clientDetails.email && clientId) {
                  try {
                    console.log(
                      `🔍 Trying direct API lookup for client ID: ${clientId}`,
                    );
                    const response = await fetch(
                      `https://taxabide.in/api/get-client.php?user_id=${userId}&client_id=${clientId}`,
                    );
                ...
                if (clientDetails.name || clientDetails.email) {
                  ...
                  console.log(
                    `⚠️ WARNING: No client data found for ID: ${clientId}`,
                  );
                }
                ...
                return {
                  ...
                  t_d_more_service:
                    item.t_d_more_service || item.more_service || '',
                  ...
                  t_d_service_type:
                    item.t_d_service_type || item.service_type || '',
          ...
          if (retryCount < 5) {
            ...
            fetchWebDevelopmentData(userIdToUse);
            return;
          }
  ...
  const TableHeader = () => (
    <View style={styles.tableHeader}>
      ...
      <Text style={[styles.headerCell, styles.moreServiceCell]}>
        More Services
      </Text>
  ...
  const TableRow = ({item, index}) => (
    <View style={styles.tableRow}>
      ...
      <Text style={[styles.tableCell, styles.emailCell]} numberOfLines={2}>
        {item.t_d_email || '-'}
      </Text>
      <Text style={[styles.tableCell, styles.phoneCell]}>
        {item.t_d_phone || '-'}
      </Text>
      <Text style={[styles.tableCell, styles.serviceCell]} numberOfLines={2}>
        {item.t_d_service || '-'}
      </Text>
      <Text
        style={[styles.tableCell, styles.moreServiceCell]}
        numberOfLines={3}>
        {item.t_d_more_service || '-'}
      </Text>
      <Text style={[styles.tableCell, styles.messageCell]} numberOfLines={3}>
        {item.t_d_msg || '-'}
      </Text>
      <Text style={[styles.tableCell, styles.clientIdCell]}>
        {item.t_d_client_id || '-'}
      </Text>
      <View style={[styles.tableCell, styles.clientNameCell]}>
        {item.client_name ? (
          ...
          <Text style={styles.clientIdText}>Client #{item.t_d_client_id}</Text>
        ) : (
          <Text>-</Text>
        )}
      </View>
      <View style={[styles.tableCell, styles.emailCellHighlight]}>
        {item.client_email ? (
          <Text style={styles.clientEmailText} numberOfLines={2}>
            {item.client_email}
          </Text>
        ) : (
  ...
  const NoData = () => (
    <View style={styles.noDataContainer}>
      ...
      <Text style={styles.noDataSubText}>
        There are no web development records to display
      </Text>
  ...
  const getAllClientsDirectly = async currentUserId => {
    ...
    try {
      ...
      console.log(
        `🔍 Searching for clients using ${endpoints.length} different APIs for user ID: ${userIdToUse}`,
      );

      // Try all endpoints in parallel using Promise.all with proper formatting
      let responses = [];
      try {
      ...
      for (let i = 0; i < responses.length; i++) {
        ...
        try {
          ...
          console.log(
            `Response from endpoint ${i + 1}:`,
            parsed ? 'Data received' : 'Empty data',
          );
          ...
          if (
            ...
            parsed.data.forEach(client => {
              if (client.c_id || client.id || client.client_id) {
                ...
                clientMap[clientId] = {
                  id: clientId,
                  name:
                    client.c_name || client.name || client.client_name || '',
                  email:
                    client.c_email || client.email || client.client_email || '',
                };
            ...
            console.log(
              `Found ${parsed.data.length} clients from endpoint ${i + 1}`,
            );
              ...
              if (client.id || client.client_id || client._id) {
                const transformedClient = {
                  ...
                  c_name:
                    ...
                    'Unknown Client',
                  c_email:
                    client.email || client.client_email || client.c_email || '',
                };
            ...
            console.log(
              `Found ${parsed.clients.length} clients from endpoint ${i + 1}`,
            );
          }
        } catch (e) {
      ...
      if (allClients.length > 0) {
        console.log(
          `✅ SUCCESS: Found a total of ${allClients.length} unique clients`,
        );
        console.log(
          `✅ SUCCESS: Created client map with ${Object.keys(clientMap).length} entries`,
        );
      ...
      // We'll keep loading true until the web development data is loaded
    }
  };
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <View style={styles.container}>
        <Text style={styles.heading}>
          Tulyarth DigiWeb Services - Web Development Data
        </Text>
          ...
          <View style={styles.paginationControls}>
            ...
            <View style={styles.pageNumberContainer}>
              <Text style={styles.pageNumberText}>
                Page {totalEntries > 0 ? currentPage : '-'} of{' '}
                {totalEntries > 0
...
export default WebDevelopmentTable;
